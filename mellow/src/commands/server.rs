...
pub async fn setup(_context: Context, interaction: Interaction) -> Result<CommandResponse> {
	...
	Ok(if server_exists(&guild_id).await? {
		...
		if let Some(user) = HAKUMI_MODELS.user_by_discord(guild_id, interaction.member.unwrap().user.unwrap().id).await? {
			CommandResponse::defer(interaction.token.clone(), Box::pin(async move {
				...
				DATABASE.from("mellow_servers")
					.insert(format!(r#"{{
						...
						"banner_url": {},
						"owner_user_id": "{}"
					}}"#, guild.name, user.id, guild.icon.map(|x| x.to_string()).unwrap_or("".into()), match guild.splash {
